    ...
    <Form.Group id={id} name={inputName} className={className}>
      ...
      >
        <Controller
          ...
          control={control}
          render={({ field }) => {
            //const maybeFindValueByLabel = _.find(options, {label: field.value ?? selectedValue })
            //const maybeFindValueById = _.find(options, {value: field.value ?? selectedValue })
            //const value = maybeFindValueByLabel ?? maybeFindValueById
            const value = _.isArray(field.value ?? selectedValue)
              ? field.value.map((v) => ({
                  value: v.id,
                  label: _.get(
                    _.find(options, { value: v.id }),
                    "label",
                    "loading.."
                  ),
                }))
            ...
            return (
              <Select
                ...
                onChange={(item) => {
                  ...
                  //return field.onChange((_.isArray(item) ? _.map(item, 'value') : []))
